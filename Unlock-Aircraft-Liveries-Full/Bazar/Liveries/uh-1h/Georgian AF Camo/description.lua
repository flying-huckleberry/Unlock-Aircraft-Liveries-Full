livery = {
	{"civ_uh1_main_dam", 0, "uh1_GRG_cam", true};
	{"civ_uh1_main_dam", 2, "uh1_spec", true};
	{"civ_uh1_tail_dam", 0, "uh1_tail_GRG_cam", true};
	{"civ_uh1_tail_dam", 2, "uh1_tail_spec", true};	
	{"civ_uh1_main", 0, "uh1_GRG_cam", true};
	{"civ_uh1_main", 2, "uh1_spec", true};
	{"civ_uh1_tail", 0, "uh1_tail_GRG_cam", true};
	{"civ_uh1_tail", 2, "uh1_tail_spec", true};	
	
	{"uh1_main_dam", 0, "uh1_GRG_cam", true};
	{"uh1_main_dam", 2, "uh1_spec", true};
	{"uh1_tail_dam", 0, "uh1_tail_GRG_cam", true};
	{"uh1_tail_dam", 2, "uh1_tail_spec", true};	
	{"uh1_main", 0, "uh1_GRG_cam", true};
	{"uh1_main", 2, "uh1_spec", true};
	{"uh1_tail", 0, "uh1_tail_GRG_cam", true};
	{"uh1_tail", 2, "uh1_tail_spec", true};	
	{"uh1_weapon", 0, "uh1_weapon", true};
	{"pilot_UH1_01", 0, "pilot_UH1_01", true};
	{"pilot_UH1_02", 0, "pilot_UH1_02", true};
	{"pilot_UH1_helmet", 0, "pilot_UH1_helmet", true};
	{"pilot_UH1_patch", 0, "empty", true};
	{"uh1_cov", 0, "uh1_grg_cover", false};
	{"UH1-cpt-door", 0 ,"UH1_door_c",true};
	{"UH1-cpt_karkas", 0 ,"UH1_karkas_c",true};
	{"UH1-cpt_karkas2", 0 ,"UH1_karkas2_c",true};
	{"UH1-cpt_panel", 0 ,"UH1_panel_c",true};
	{"UH1-cpt_central_box", 0 ,"UH1_boxs_c",true};
	{"UH1-cpt_dev2", 0 ,"UH1_dev_2",true};
}

